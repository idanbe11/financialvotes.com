...
export async function getTodaysBestCoins() {
  const data = await fetchAPI('/coins/today');
  return data;
}

export async function getAllTimeBestCoins() {
  const data = await fetchAPI('/coins?_sort=votes:DESC');
  return data;
}

export async function getAllPromotedCoins() {
  const data = await fetchAPI('/coins?promoted=true&active=true');
  return data;
}
...
export async function getNewsData(page = 1) {
  const data = await fetchAPIUrl(
    `https://newsdata.io/api/1/news?apiKey=pub_7030902f1ddb96056927cb89376eaadba0d&from_date=2021-10-20&q=crypto%20OR%20coin&language=en&page=${page}`
  );
  return data;
}
